...
import {
  UserPlus, Pencil, Trash2, Eye, EyeOff, X, ShieldAlert, KeyRound,
} from 'lucide-react'
...
export default function Settings() {
  ...
  const [pwUser, setPwUser] = useState<UserOut | null>(null)
  ...
  function openPasswordModal(user: UserOut) {
    setPwUser(user)
    setPwValue('')
    setPwConfirm('')
    setPwShow(false)
  ...
  async function handlePasswordReset() {
    ...
    try {
      ...
      setPwError(extractApiError(err, 'Error al cambiar la contraseña'))
    } finally {
      setPwSaving(false)
    }
  }
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white rounded-card shadow-card overflow-hidden">
        {/* Table head */}
        <div className="grid grid-cols-[2fr_1fr_1.4fr_1.4fr_1fr_140px] bg-brand-dark text-white">
        ...
        {users.map((user) => (
          <div
            key={user.id}
            className="grid grid-cols-[2fr_1fr_1.4fr_1.4fr_1fr_140px] border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors"
          >
            {/* User info */}
                ...
                {deleteConfirm === user.id ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-md border border-gray-200 flex items-center justify-center text-gray-400 hover:border-amber-300 hover:text-amber-500 transition-colors"
                    >
      ...
      {pwUser && (
        <div className="fixed inset-0 bg-brand-dark/45 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-card p-8 w-[420px] shadow-2xl">
            <div className="flex items-start justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-semibold text-brand-ink">Cambiar contraseña</h2>
                  <p className="text-xs text-brand-muted mt-0.5">
                    @{pwUser.username}{pwUser.full_name ? ` · ${pwUser.full_name}` : ''}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-brand-muted">
                La nueva contraseña debe tener al menos 12 caracteres e incluir mayúsculas, minúsculas, un número y un símbolo especial.
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
              ...
              <button
                ...
                disabled={pwSaving || !pwValue || !pwConfirm}
                className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white rounded-btn px-5 py-2 text-sm font-semibold transition-colors disabled:opacity-60"
              >
                <KeyRound className="w-3.5 h-3.5" />
